...
3. **Verification Process**
   - Improvements are verified on-chain by **DeltaVerifier**, which consumes **HEM** (Hokusai Evaluation Manifest) per-row eval artifacts persisted to MLflow
   - Accepted gains trigger a **MintRequest** (schema v1.0) published to Redis, standardized on bps + USDC micro-units
