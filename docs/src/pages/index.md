---
sidebar_position: 1
---

# Introduction

Welcome to the Hokusai Protocol documentation. Hokusai is a decentralized protocol that creates fair incentives for training smarter AI models.

## What is Hokusai?

Hokusai Protocol is a decentralized system that:

- Enables data suppliers to earn rewards for contributing high-quality training data
- Allows AI developers to access performant models and create their own tokens
- Creates a sustainable ecosystem for AI model development and data contribution

## Key Features

- **Hokusai Tokens**: ERC20 tokens that represent AI model performance. 
- **Data Rewards**: Earn rewards for contributing high-quality training data
- **Model Access**: Access performant models through the protocol
- **Decentralized Governance**: Community-driven protocol development and decision making

## Getting Started

- [Getting Started as a Data Supplier](/supplying-data)
- [Getting Started as a AI Developer](/ai-developers)
- [Investors should understand Tokenomics](/tokenomics)
- [Technical Architecture](/architecture)

For further assistance, refer to the FAQs or join our community discussions.